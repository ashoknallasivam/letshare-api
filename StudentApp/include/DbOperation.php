...
class DbOperation
{
    ...
    public function createUser($name,$username,$pass){
        if (!$this->isUserExists($username)) {
			...
			$sql = "INSERT INTO `user`(`token`,`name`, `username`,`password`,`api_key`,`status`,`created_on`) VALUES (:token,:name,:username,:password,:api_key,:status,:created_on)";
			$pre  = $this->con->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
			$values = array(
			':token' => $token,
			':name' => $name,
			':username' => $username,
    ...
    public function getUser($username){
		...
		':username' => $username);
		$pre->execute($values);
		$student = $pre->fetch(PDO::FETCH_ASSOC);
        return $student;
    }
